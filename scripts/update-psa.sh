...
if [ ! -d "$PSA_DIR/.git" ]; then
    ...
    echo ""
    echo "  1. Backup: cp -r ~/.psa ~/.psa.backup"
    echo "  2. Remove: rm -rf ~/.psa"
    echo "  3. Clone: git clone https://github.com/bradheitmann/psa.git ~/.psa"
    echo "  4. Install: cd ~/.psa && ./install.sh"
    echo ""
    exit 1
fi
